...
get_downstream_genes_per_receptor <- function(){
...
# filter empty results
  ...
  named_symbol_list <- combined_Baseline_vs_t24h_perct_omni_unweighted_target_values[names(named_symbol_list) %in% named_list_mapping[['SYMBOL']]]
  # swap to entrez
  ...
  names(named_symbol_list) <- named_symbol_list
  return(named_symbol_list)
}


pathwview_per_celltype_combination <- function(nichenet_output, output_loc, mast_output_loc, pathways.ids, organism='org.Hs.eg.db'){
  # check each receiver
  for(receiver in names(nichenet_output)){
    # check against each sender
    for(sender in names(nichenet_output[[receiver]])){
      ...
      if('ligand_receptor_network' %in% names(nichenet_output[[receiver]][[sender]])){
        ...
        if(nrow(ligand_receptor_matrix) > 0){
          ...
          # switch entrez
          ligand_lr_sums <- named_symbol_list_to_entrez(ligand_lr_sums)
          receptor_sums <- named_symbol_list_to_entrez(receptor_sums)
...
pathview(gene.data=c(ligand_lr_sums, receptor_sums), pathway.id = pathway.id, species = kegg.organism)
          }
...
pathwview_per_celltype_combination(nichenet_output = v2_Baseline_vs_t24h_perct_omni_unweighted, output_loc = './', pathways.ids = c('hsa04060','hsa04061','hsa04657','hsa04659','hsa04620','hsa05321','hsa05417'))
