...
limma_to_niche_output <- function(limma_output_loc, cell_types = c('B', 'CD4T', 'CD8T', 'DC', 'NK', 'monocyte'), niches=c('Baseline', 't24h', 't8w'), midpend='_timepoint.final.', append='.tsv', pval_column='', pval_cutoff=0.05, logfc_threshold=0.1) {
  # we'll save it in a list
  de_per_niche_combo <- list()
  # we'll check each cell type
  for (cell_type in cell_types) {
    # and each niche
    for (niche1 in niches) {
      # against each other niche
      for (niche2 in niches) {
        # paste together the path
        limma_full_path <- paste(limma_output_loc, '/', cell_type, midpend, niche1, '_', niche2, append, sep = '')
        # check if that exists
        if (file.exists(limma_full_path)) {
          # read the file
          output_niches <- read.table(limma_full_path, sep = '\t', header = T, row.names = 1)
          if (nrow(output_niches) > 0) {
            nn_de_table <- data.frame(gene = rownames(output_niches), p_val = output_niches[['P.Value']], avg_log2FC = output_niches[['logFC']], pct.1 = 0, pct.2 = 0, p_val_adj = output_niches[['adj.P.Val']], sender = paste(cell_type, niche1, sep = '_'), sender_other_niche = paste(cell_type, niche2, sep = '_'))
            # filter
            nn_de_table[nn_de_table[[pval_column]] < pval_cutoff, ]
            # if there is anything left, add to the list
            if (nrow(nn_de_table) > 0) {
              de_per_niche_combo[[paste(cell_type, niche1, niche2)]] <- nn_de_table
            }
          }
        }
      }
  ...
  # merge all
  de_per_niche_combo_all <- do.call('rbind', de_per_niche_combo)
  ...
  for (ct1 in cell_types) {
    ...
    for (ct2 in cell_types) {
      ...
      for (niche1 in niches) {
        ...
        for (niche2 in niches) {
          # paste together the path
          limma_full_path <- paste(limma_output_loc, '/', prepend, ct1, '_', niche1, '_', ct2, '_', niche2, append, sep = '')
          # check if that exists
          if (file.exists(limma_full_path)) {
            # read the file
            output_niches <- read.table(limma_full_path, sep = '\t', header = T, row.names = 1)
            if (nrow(output_niches) > 0) {
              nn_de_table <- data.frame(gene = rownames(output_niches), p_val = output_niches[['P.Value']], avg_log2FC = output_niches[['logFC']], pct.1 = 0, pct.2 = 0, p_val_adj = output_niches[['adj.P.Val']], sender = paste(ct1, niche1, sep = '_'), sender_other_niche = paste(ct2, niche2, sep = '_'))
              # filter
              nn_de_table[nn_de_table[[pval_column]] < pval_cutoff, ]
              # if there is anything left, add to the list
              if (nrow(nn_de_table) > 0) {
                ...
                # add that as well
                de_per_niche_combo[[paste(ct2, ct1, niche2, niche1)]] <- nn_de_table_opposite
              }
            }
          }
        }
...
# parameters
seurat_obj=cardio_integrated
celltype_column='cell_type_lowerres'
condition_column='timepoint.final'
condition_1='t24h'
condition_2='Baseline'
cell_types_of_interest=c('B', 'CD4T', 'CD8T', 'DC', 'monocyte', 'NK')
assay_oi='SCT'
expression_pct=0.1
test_use='wilcox'
