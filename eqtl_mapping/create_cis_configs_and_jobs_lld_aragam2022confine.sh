...
GENOTYPE_LOC='/groups/umcg-bios/tmp01/projects/1M_cells_scRNAseq/ongoing/genotypes/LL_tritypes/'
...
do
  ...
  if [[ ${f} =~ .*\.(tsv) ]]
  then
    # ${f} is the features file full path
 we also need just the name
    # echo "${f}"
