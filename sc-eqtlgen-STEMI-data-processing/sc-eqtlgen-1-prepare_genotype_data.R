...
# turn into pfile
#singularity exec --bind /groups/umcg-franke-scrna/tmp01/projects/sc-eqtlgen-consortium-pipeline/ongoing/wg1-preprocessing/wg1_blokland2020/stemi_v2/genotype/unimputed/ /groups/umcg-wijmenga/tmp01/users/umcg-roelen/singularity/single_cell_container/singlecell_container.simg plink2 --vcf /groups/umcg-franke-scrna/tmp01/projects/sc-eqtlgen-consortium-pipeline/ongoing/wg1-preprocessing/wg1_blokland2020/stemi_v2/genotype/unimputed/stemi_v2_unimputed.vcf.gz --make-pgen --out /groups/umcg-franke-scrna/tmp01/projects/sc-eqtlgen-consortium-pipeline/ongoing/wg1-preprocessing/wg1_blokland2020/stemi_v2/genotype/unimputed/stemi_v2_unimputed
...
v2_original_psam_loc <- '/groups/umcg-franke-scrna/tmp01/projects/sc-eqtlgen-consortium-pipeline/ongoing/wg1-preprocessing/wg1_blokland2020/stemi_v2/genotype/unimputed/stemi_v2_unimputed.psam.original'
v2_new_psam_loc <- '/groups/umcg-franke-scrna/tmp01/projects/sc-eqtlgen-consortium-pipeline/ongoing/wg1-preprocessing/wg1_blokland2020/stemi_v2/genotype/unimputed/stemi_v2_unimputed.psam'
v2_psam <- read.table(v2_original_psam_loc, header = F, sep = '\t')
colnames(v2_psam) <- c('#FID', 'SEX')
# we remove the sex column, as it is empty right now
v2_psam[['SEX']] <- NULL
# add the other ID column
v2_psam[['IID']] <- v2_psam[['#FID']]
# needs to be numeric?
v2_psam[['PAT']] <- 0
v2_psam[['MAT']] <- 0
# grab the sex from the metadata
v2_psam_sex <- metadata[match(v2_psam[['IID']], metadata[['ID']]), 'gender']
# create the empty sex column in the psam
v2_psam[['SEX']] <- NA
# we need to change the coding from M/F to 1/2 in the psam
v2_psam[v2_psam_sex == 'M', 'SEX'] <- 1
v2_psam[v2_psam_sex == 'F', 'SEX'] <- 2
# we don't know most of these
v2_psam[['Provided_Ancestry']] <- 'EUR'
v2_psam[['genotyping_platform']] <- 'cytoSNP'
v2_psam[['array_available']] <- 'N'
v2_psam[['wgs_available']] <- 'N'
v2_psam[['wes_available']] <- 'Y'
v2_psam[['age']] <- metadata[match(v2_psam[['IID']], metadata[['ID']]), 'age']
v2_psam[['age_range']] <- apply(v2_psam, 1, function(x){
  floor(as.numeric(x['age'])/10)*10
})
v2_psam[['Study']] <- 'stemi_v2'
v2_psam[['smoking_status']] <- NA
v2_psam[['hormonal_contraception_use_currently']] <- NA
v2_psam[['menopause']] <- NA
v2_psam[['pregnancy_status']] <- NA
write.table(v2_psam, v2_new_psam_loc, sep = '\t', row.names = F, col.names = T, quote = F)
