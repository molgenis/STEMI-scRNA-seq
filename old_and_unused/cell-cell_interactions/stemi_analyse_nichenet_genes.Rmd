...
interactions_to_circle_from_table <- function(interactions_table, plot_title='cell communication', combine_directions=T){
  # we'll use colors
  color_coding <- get_color_coding_dict()
  ...
  # if we are not combining directions, we need to make sender and receiver unique
  if (!combine_directions) {
    interactions_table[['sender']] <- paste(interactions_table[['sender']], 'sent', sep = '_')
    interactions_table[['receiver']] <- paste(interactions_table[['receiver']], 'received', sep = '_')
    # we also need to send those as colours
    color_coding[paste(names(get_color_coding_dict()), 'sent', sep = '_')] <- as.vector(unlist(get_color_coding_dict()))
    color_coding[paste(names(get_color_coding_dict()), 'received', sep = '_')] <- as.vector(unlist(get_color_coding_dict()))
  }
  ...
  for(cell_type in unique(connections_all_ct$a)){
    if (combine_directions) {
      ...
      # take the label dict
      label_dict_modified <- label_dict()
      # add the keys for senders and receivers
      label_dict_modified[paste(names(label_dict()), 'sent', sep = '_')] <- paste(as.vector(unlist(label_dict())), 'sent', sep = ' ')
      label_dict_modified[paste(names(label_dict()), 'received', sep = '_')] <- paste(as.vector(unlist(label_dict())), 'received', sep = ' ')
          highlight.sector(c(cell_type), track.index = 1, text=label_dict_modified[[cell_type]], col = '#ffffff00', text.col = '#ffffffff')
    }
  }
  ...
  apply(connections_start_stop, 1, function(row){
    if(!is.na(row['sender'])){
      ...
      send_color <- color_coding[[row['sender']]]
      # and of the receiver
      receiver_color <- color_coding[[row['receiver']]]
...
lr_pairs_per_celtype <- function(lr_table, celltype_column, ligand_column='ligand', receptor_column='receptor') {
  ...
  for (celltype in unique(lr_table[[celltype_column]])) {
    ...
    # put in that list
    lr_per_celltype[[celltype]] <- lr_connections_celltype
  }
...
receptors_per_celltype <- function(lr_table, celltype_column, receptor_column='receptor') {
  # put per celltype in a list
  receptors_per_celltype <- list()
  # check each celltype
  for (celltype in unique(lr_table[[celltype_column]])) {
    # get the unique receptors
    receptors <- unique(lr_table[lr_table[[celltype_column]] == celltype, receptor_column])
    # add to list
    receptors_per_celltype[[celltype]] <- receptors
  }
  return(receptors_per_celltype)
}
...
mast_output_loc <- '/groups/umcg-franke-scrna/tmp01/releases/blokland-2020/v1/differential_expression/MAST/stemi_meta_paired_lores_lfc01minpct01ncountrna_20210301/rna/'
...
ligand_target_matrix_omni_unweighted_loc <-paste('/groups/umcg-franke-scrna/', read_partition, '/releases/blokland-2020/v1/cell_cell_interactions/omni/', 'ligand_target_matrixNoweights.rds', sep = '')
lr_network_omni_loc <- paste('/groups/umcg-franke-scrna/', read_partition, '/releases/blokland-2020/v1/cell_cell_interactions/omni/','lr_Network_Omnipath.rds', sep = '')
unweighted_networks_omni_loc <- paste('/groups/umcg-franke-scrna/', read_partition, '/releases/blokland-2020/v1/cell_cell_interactions/omni/','weighted_networksNonSourceWeights.rds', sep = '')
# there are weighted matrices as well
ligand_target_matrix_omni_weighted_loc <-paste('/groups/umcg-franke-scrna/', read_partition, '/releases/blokland-2020/v1/cell_cell_interactions/omni/', 'ligand_target_matrixWithweights.rds', sep = '')
weighted_networks_omni_loc <- paste('/groups/umcg-franke-scrna/', read_partition, '/releases/blokland-2020/v1/cell_cell_interactions/omni/','weighted_networksWithSourceWeights.rds', sep = '')
# read network data
...
lr_filtered_Baseline_t24h_v3 <- filter_lr_by_lt_ligands(data.frame(lt_v3_Baseline_vs_t24h_perct_omni_unweighted), lr_connections = data.frame(lr_v3_Baseline_vs_t24h_perct_omni_unweighted), target_interaction_column = 'v3_Baseline_vs_t24h', receptor_interaction_column = 'v3_Baseline_vs_t24h')
lr_filtered_Baseline_t8w_v3 <- filter_lr_by_lt_ligands(data.frame(lt_v3_Baseline_vs_t8w_perct_omni_unweighted), lr_connections = data.frame(lr_v3_Baseline_vs_t8w_perct_omni_unweighted), target_interaction_column = 'v3_Baseline_vs_t8w', receptor_interaction_column = 'v3_Baseline_vs_t8w')
lr_filtered_t24h_t8w_v3 <- filter_lr_by_lt_ligands(data.frame(lt_v3_t24h_vs_t8w_perct_omni_unweighted), lr_connections = data.frame(lr_v3_t24h_vs_t8w_perct_omni_unweighted), target_interaction_column = 'v3_t24h_vs_t8w', receptor_interaction_column = 'v3_t24h_vs_t8w')
lr_filtered_Baseline_t24h_v2 <- filter_lr_by_lt_ligands(data.frame(lt_v2_Baseline_vs_t24h_perct_omni_unweighted), lr_connections = data.frame(lr_v2_Baseline_vs_t24h_perct_omni_unweighted), target_interaction_column = 'v2_Baseline_vs_t24h', receptor_interaction_column = 'v2_Baseline_vs_t24h')
lr_filtered_Baseline_t8w_v2 <- filter_lr_by_lt_ligands(data.frame(lt_v2_Baseline_vs_t8w_perct_omni_unweighted), lr_connections = data.frame(lr_v2_Baseline_vs_t8w_perct_omni_unweighted), target_interaction_column = 'v2_Baseline_vs_t8w', receptor_interaction_column = 'v2_Baseline_vs_t8w')
lr_filtered_t24h_t8w_v2 <- filter_lr_by_lt_ligands(data.frame(lt_v2_t24h_vs_t8w_perct_omni_unweighted), lr_connections = data.frame(lr_v2_t24h_vs_t8w_perct_omni_unweighted), target_interaction_column = 'v2_t24h_vs_t8w', receptor_interaction_column = 'v2_t24h_vs_t8w')

# merge all of them
lr_filtered_all <- merge(lr_filtered_Baseline_t24h_v2, lr_filtered_Baseline_t24h_v3, by = c('receiver', 'sender', 'ligand', 'receptor'), all = T)
lr_filtered_all <- merge(lr_filtered_all, lr_filtered_Baseline_t8w_v2, by = c('receiver', 'sender', 'ligand', 'receptor'), all = T)
lr_filtered_all <- merge(lr_filtered_all, lr_filtered_Baseline_t8w_v3, by = c('receiver', 'sender', 'ligand', 'receptor'), all = T)
lr_filtered_all <- merge(lr_filtered_all, lr_filtered_t24h_t8w_v2, by = c('receiver', 'sender', 'ligand', 'receptor'), all = T)
lr_filtered_all <- merge(lr_filtered_all, lr_filtered_t24h_t8w_v3, by = c('receiver', 'sender', 'ligand', 'receptor'), all = T)
```
...
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(ligand_connection_numbers_v3_Baseline_vs_t24h_perct_omni_unweighted, plot_title = 'communicating ligands t0 vs t24h (v3 chemistry)', combine_directions = T)
```

```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(ligand_connection_numbers_v3_Baseline_vs_t24h_perct_omni_unweighted, plot_title = 'communicating ligands t0 vs t24h (v3 chemistry)', combine_directions = F)
```
...
```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(ligand_connection_numbers_v3_Baseline_vs_t8w_perct_omni_unweighted, plot_title = 'communicating ligands t0 vs t6-8w (v3 chemistry)', combine_directions = F)
```
...
```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(ligand_connection_numbers_v3_t24h_vs_t8w_perct_omni_unweighted, plot_title = 'communicating ligands t24h vs t6-8w (v3 chemistry)', combine_directions = F)
```
...
```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(downstream_gene_nrs_Baseline_vs_t24h[, c('sender', 'receiver', 'nr_downstream_genes')], plot_title = 'induced DE t0 vs t24h (v3 chemistry)', combine_directions = F)
```
...
```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(downstream_gene_nrs_Baseline_vs_t8w[, c('sender', 'receiver', 'nr_downstream_genes')], plot_title = 'induced DE t0 vs t6-8w (v3 chemistry)', combine_directions = F)
```
...
```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(downstream_gene_nrs_t24h_vs_t8w[, c('sender', 'receiver', 'nr_downstream_genes')], plot_title = 'induced DE t24h vs t6-8w (v3 chemistry)', combine_directions = F)
```
...
```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(lt_nrs_Baseline_vs_t24h[, c('sender', 'receiver', 'nr_lt_links')], plot_title = 'active ligand-target links\nt0 vs t24h (v3 chemistry)', combine_directions = F)
```
...
```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(lt_nrs_Baseline_vs_t8w[, c('sender', 'receiver', 'nr_lt_links')], plot_title = 'active ligand-target links\nt0 vs t6-8w (v3 chemistry)', combine_directions = F)
```
...
```{r, include=TRUE, fig.width=4, fig.height=4}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(lt_nrs_t24h_vs_t8w[, c('sender', 'receiver', 'nr_lt_links')], plot_title = 'active ligand-target links\nt24h vs t6-8w (v3 chemistry)', combine_directions = F)
```

```{r, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

# now based on the combinations of ligand and receptor
lr_nrs_Baseline_vs_t24h <- ligand_receptor_numbers_to_interactions(r_vs_t_v3_Baseline_vs_t24h_perct_omni_unweighted)
lr_nrs_Baseline_vs_t8w <- ligand_receptor_numbers_to_interactions(r_vs_t_v3_Baseline_vs_t8w_perct_omni_unweighted)
lr_nrs_t24h_vs_t8w <- ligand_receptor_numbers_to_interactions(r_vs_t_v3_t24h_vs_t8w_perct_omni_unweighted)

```

```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(lr_nrs_Baseline_vs_t24h[, c('sender', 'receiver', 'nr_connections')], plot_title = 'active ligand-receptor links\nt0 vs t24h (v3 chemistry)')
```

```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(lr_nrs_Baseline_vs_t8w[, c('sender', 'receiver', 'nr_connections')], plot_title = 'active ligand-receptor links\nt0 vs t8w (v3 chemistry)')
```

```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(lr_nrs_t24h_vs_t8w[, c('sender', 'receiver', 'nr_connections')], plot_title = 'active ligand-receptor links\nt24h vs t8w (v3 chemistry)')
```
...
```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(lr_nrs_Baseline_vs_t8w[, c('sender', 'receiver', 'nr_connections')], plot_title = 'active ligand-receptor links\nt0 vs t8w (v3 chemistry)' ,combine_directions = F)
```

```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
interactions_to_circle_from_table(lr_nrs_t24h_vs_t8w[, c('sender', 'receiver', 'nr_connections')], plot_title = 'active ligand-receptor links\nt24h vs t8w (v3 chemistry)' ,combine_directions = F)
```

```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
lr_overlap_received_v3_Baseline_vs_t24h <- lr_pairs_per_celtype(lr_filtered_Baseline_t24h_v3, celltype_column = 'receiver')
lr_pairs_to_upset(lr_overlap_received_v3_Baseline_vs_t24h)
grid.text("LR-sharedness per receiving celltype Baseline-t24h (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=15))
```

```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
lr_overlap_received_v3_Baseline_vs_t8w <- lr_pairs_per_celtype(lr_filtered_Baseline_t8w_v3, celltype_column = 'receiver')
lr_pairs_to_upset(lr_overlap_received_v3_Baseline_vs_t8w)
grid.text("LR-sharedness per receiving celltype Baseline-t8w (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=15))
```
```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
lr_overlap_received_v3_t24h_vs_t8w <- lr_pairs_per_celtype(lr_filtered_t24h_t8w_v3, celltype_column = 'receiver')
lr_pairs_to_upset(lr_overlap_received_v3_t24h_vs_t8w)
grid.text("LR-sharedness per receiving celltype t24h-t8w (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=15))
```

```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
l_overlap_received_v3_Baseline_vs_t24h <- ligands_per_celltype(lr_filtered_Baseline_t24h_v3, celltype_column = 'receiver')
lr_pairs_to_upset(l_overlap_received_v3_Baseline_vs_t24h, use_label_dict = T, use_color_dict = T)
grid.text("ligand-sharedness per receiving celltype Baseline-t24h (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=12))
```

```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
l_overlap_received_v3_Baseline_vs_t8w <- ligands_per_celltype(lr_filtered_Baseline_t8w_v3, celltype_column = 'receiver')
lr_pairs_to_upset(l_overlap_received_v3_Baseline_vs_t8w)
grid.text("ligand-sharedness per receiving celltype Baseline-t8w (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=12))
```
```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
l_overlap_received_v3_t24h_vs_t8w <- ligands_per_celltype(lr_filtered_t24h_t8w_v3, celltype_column = 'receiver')
lr_pairs_to_upset(l_overlap_received_v3_t24h_vs_t8w)
grid.text("ligand-sharedness per receiving celltype t24h-t8w (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=12))
```
```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
r_overlap_received_v3_Baseline_vs_t24h <- receptors_per_celltype(lr_filtered_Baseline_t24h_v3, celltype_column = 'receiver')
lr_pairs_to_upset(r_overlap_received_v3_Baseline_vs_t24h, use_label_dict = T, use_color_dict = T)
grid.text("receptor-sharedness per receiving celltype Baseline-t24h (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=12))
```
```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
r_overlap_received_v3_Baseline_vs_t8w <- receptors_per_celltype(lr_filtered_Baseline_t8w_v3, celltype_column = 'receiver')
lr_pairs_to_upset(r_overlap_received_v3_Baseline_vs_t8w, use_label_dict = T, use_color_dict = T)
grid.text("receptor-sharedness per receiving celltype Baseline-t8w (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=12))
```
```{r, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)
r_overlap_received_v3_t24h_vs_t8w <- receptors_per_celltype(lr_filtered_t24h_t8w_v3, celltype_column = 'receiver')
lr_pairs_to_upset(r_overlap_received_v3_t24h_vs_t8w, use_label_dict = T, use_color_dict = T)
grid.text("receptor-sharedness per receiving celltype t24h-t8w (V3 chemistry)",x = 0.65, y=0.95, gp=gpar(fontsize=12))
```
