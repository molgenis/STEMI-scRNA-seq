---
title: "stemi_factor_replication_preprocess"
author: "Roy Oelen"
date: "2023-02-20"
...
# Name: stemi_factor_replication_preprocess.Rmd
# Function:
############################################################################################################################
...
library(Seurat)
library(SeuratDisk)
library(muscat) # NOT IN CONTAINER
library(reshape2)
...
seurat_input_data_name = '/groups/umcg-franke-scrna/tmp01/releases/blokland-2020/v1/seurat/cardio.integrated.20210301.rds'
#name_save = 'V_AZIMUTH_REPRODUCTION'
name_save = 'V_AZIMUTH_REPRODUCTION_v2'

## Define columns in seurat object containing sample-id and cluster annotations

sample_column = 'timepoint.assignment.final' # to be sample-id
cluster_column =  'cell_type' # to be cluster_id; cell-type annotation
...
```{r load_object, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

###### Load the generated seurat objects

source_text = paste(seurat_input_data_name)
print(source_text)
print(file.info(source_text)$mtime)
rna_seurat_data = readRDS(source_text)
# add a new column
rna_seurat_data@meta.data[['timepoint.assignment.final']] <- paste(rna_seurat_data@meta.data[['timepoint.final']], rna_seurat_data@meta.data[['assignment.final']], sep = '.')

# subset chemistry
rna_seurat_data <- rna_seurat_data[, rna_seurat_data@meta.data[['chem']] == 'V2']
...
DefaultAssay(rna_seurat_data_subset) <- 'RNA'
rm(rna_seurat_data)
...
```{r genes_clusters, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
...
if (name_save == 'V_AZIMUTH_REPRODUCTION_v3') {
  ##### Condition for filtering genes
cell_perc_cluster =  cell_perc_cluster[((cell_perc_cluster$perc_cells > 50) & (cell_perc_cluster$total_amount_cells_expressing_gene >400)) | ((cell_perc_cluster$perc_cells > 40) & (cell_perc_cluster$total_amount_cells_expressing_gene > 1000)) ,] 
} else if(name_save == 'V_AZIMUTH_REPRODUCTION_v2') {
  ##### Condition for filtering genes
cell_perc_cluster =  cell_perc_cluster[((cell_perc_cluster$perc_cells > 30) & (cell_perc_cluster$total_amount_cells_expressing_gene >1000)) | ((cell_perc_cluster$perc_cells > 20) & (cell_perc_cluster$total_amount_cells_expressing_gene > 2500)) ,] 
} else {
...
```{r cluster_pseudobulk, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
