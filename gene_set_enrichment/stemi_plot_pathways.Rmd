...
plot_pathways_celltype_dots <- function(plot_frame, condition_column='condition', term_column='Term', significance_column='significance', set_size_column='Overlap', direction_column=NULL, set_size_limits=NULL, use_label_dict=T, pointless=F, legendless=F, paper_style=T, term_text_remove=' R-HSA-\\d+', convert_set_size=T, condition_order=NULL, low_color='darkblue', mid_color='white', high_color='darkred', split_up_down=F) {
  # use prettier labels if requested
  if(use_label_dict){
    plot_frame[[condition_column]] <- as.vector(unlist(label_dict()[plot_frame[[condition_column]]]))
  }
  ...
  if (!is.null(condition_order)) {
    plot_frame[[condition_column]] <- factor(plot_frame[[condition_column]], levels = condition_order)
  }
  # clean terms
  if (!is.null(term_text_remove)) {
    # remove some text
  ...
  # make the set size numeric from the characters, if they are not already
  if (convert_set_size) {
  ...
  # order by the significance
  plot_frame[[term_column]] <- factor(plot_frame[[term_column]], levels = rev(unique(plot_frame[order(plot_frame[[condition_column]], plot_frame[[set_size_column]]), term_column])))
  # split up and down first
  if (split_up_down) {
    # we'll save the terms
    terms_present_up <- c()
    terms_present_down <- c()
    # get per condition
    for (condition in plot_frame[[condition_column]]) {
      terms_up <- as.character(plot_frame[plot_frame[[condition_column]] == condition & plot_frame[[direction_column]] == 'up', term_column])
      terms_down <- as.character(plot_frame[plot_frame[[condition_column]] == condition & plot_frame[[direction_column]] == 'down', term_column])
      terms_present_up <- c(terms_present_up, terms_up)
      terms_present_down <- c(terms_present_down, terms_down)
    }
    # now deduplicate them
    terms_present_up <- terms_present_up[!duplicated(terms_present_up)]
    terms_present_down <- terms_present_down[!duplicated(terms_present_down)]
    # merge them
    terms_all <- c(terms_present_up, terms_present_down)
    # and remove duplicates again
    terms_all <- terms_all[!duplicated(terms_all)]
    # set the levels now
    plot_frame[[term_column]] <- factor(plot_frame[[term_column]], levels = rev(terms_all))
  }
  # make plot
...
# make it so that the zero ones are the minimal possible value
mast_both_enrichr_positive_mono_filtered[mast_both_enrichr_positive_mono_filtered[['significance']] == 0, 'significance'] <- .Machine$double.xmin
# and log transform
...
mast_both_enrichr_negative_mono_filtered[mast_both_enrichr_negative_mono_filtered[['significance']] == 0, 'significance'] <- .Machine$double.xmin
