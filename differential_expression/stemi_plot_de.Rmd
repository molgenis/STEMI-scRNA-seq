...
label_dict <- function(){
  ...
  label_dict[['BaselineUT']] <- 't0-C'
  label_dict[['t24hBaseline']] <- 't24h-t0'
  label_dict[['t8wBaseline']] <- 't8w-t0'
  label_dict[['t8wt24h']] <- 't8w-t24h'
  # conditions
  label_dict[['UT']] <- 'C'
  label_dict[['C']] <- 'C'
...
plot_de_number_bars <- function(numbers_table, cell_type_column='cell_type', combination_column='combination', number_column='number', direction_column='direction', cell_types_include=NULL, combinations_include=NULL, grid_single=T, use_label_dict=T, pointless=F, legendless=F, paper_style=T, angle_x_labels=F, colour_top_label=T, use_colour_coding_dict=T){
  ...
  numbers_table_to_do[['combination']] <- numbers_table_to_do[[combination_column]]
...
# show overlap of controls
p_de_overlap_controls_limma <- plot_loose_overlap_cell_types(limma_output_loc, condition_combinations = c('UT_Baseline'), de_method = 'limma', pval_column = 'p.bonferroni', pval_column_other = 'p.bonferroni', lfc_column = 'logFC', lfc_cutoff = 1)
# show overlap of STEMI timepoints
p_de_overlap_stemi_limma <- plot_loose_overlap_cell_types(limma_output_loc, condition_combinations = c('t24h_Baseline', 't8w_Baseline'), de_method = 'limma', pval_column = 'p.bonferroni', pval_column_other = 'p.bonferroni', lfc_column = 'logFC', lfc_cutoff = 1)
...
```{r mast_lfc01_de_numbers, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)

# get a table of the number of DE genes
de_numbers_table_lfc01 <- de_genes_number_to_table(mast_meta_output_loc_lfc01, lfc_cutoff = 0.1)

# modify so that the column names are a bit different against Baseline/t0
de_numbers_table_vs_t0_lfc01 <- data.frame(t24h = de_numbers_table_lfc01[['Baselinet24h']], t8w = de_numbers_table_lfc01[['Baselinet8w']], UT = de_numbers_table_lfc01[['UTBaseline']])
rownames(de_numbers_table_vs_t0_lfc01) <- rownames(de_numbers_table_lfc01)
# plot the number of DE genes
p_de_nrs_stemi_lfc01 <- numbers_table_to_plot(de_numbers_table_vs_t0_lfc01, use_groups_dict = F, cols_include = c('t24h', 't8w'), pointless = F, paper_style = T, title = 'Number of DE genes per cell type in STEMI over time', legendless = T, angle_x_labels = T) + xlab('timepoint versus STEMI t0')
# only HC vs t0
p_de_nrs_controls_lfc01 <- numbers_table_to_plot(de_numbers_table_vs_t0_lfc01, use_groups_dict = F, cols_include = c('UT'), pointless = F, title = 'Number of DE genes per cell type in Controls vs STEMI t0', legendless = T, paper_style = T) + xlab('timepoint versus STEMI t0')

# show overlap of controls
p_de_overlap_controls_lfc01 <- plot_loose_overlap_cell_types(mast_meta_output_loc_lfc01, condition_combinations = c('UTBaseline'), lfc_cutoff = 0.1)
# show overlap of STEMI timepoints
p_de_overlap_stemi_lfc01 <- plot_loose_overlap_cell_types(mast_meta_output_loc_lfc01, condition_combinations = c('Baselinet24h', 'Baselinet8w'), lfc_cutoff = 0.1)
...
plot_grid(
  plot_grid(
    ...
    p_de_nrs_controls_lfc01 + ggtitle('') + xlab('versus STEMI t0'),
    rel_heights = c(0.1, 1),
    nrow = 2
  ),
  plot_grid(
    ...
    p_de_overlap_controls_lfc01 + ggtitle(''),
    rel_heights = c(0.1, 1),
    nrow = 2
  ),
  plot_grid(
  ...
  plot_grid(
    ...
    p_de_overlap_stemi_lfc01 + ggtitle(''),
    rel_heights = c(0.1, 1),
    nrow = 2
  ),
  nrow = 2,
...
# get a table of the number of DE genes
de_numbers_table_limma_up <- de_genes_number_to_table(limma_output_loc, de_method = 'limma', pval_column = 'p.bonferroni', lfc_column = 'logFC', lfc_cutoff = 0.1, only_positive = T)
de_numbers_table_limma_up[is.na(de_numbers_table_limma_up)] <- 0
de_numbers_table_limma_down <- de_genes_number_to_table(limma_output_loc, de_method = 'limma', pval_column = 'p.bonferroni', lfc_column = 'logFC', lfc_cutoff = 0.1, only_negative = T)
de_numbers_table_limma_down[is.na(de_numbers_table_limma_down)] <- 0

# turn into long tables
de_numbers_table_limma_up_wide <- wide_to_long_table(de_numbers_table_limma_up)
de_numbers_table_limma_down_wide <- wide_to_long_table(de_numbers_table_limma_down)

# set column names
colnames(de_numbers_table_limma_up_wide) <- c('cell_type', 'combination', 'number')
colnames(de_numbers_table_limma_down_wide) <- c('cell_type', 'combination', 'number')


# add new column
de_numbers_table_limma_up_wide[['direction']] <- 'up'
de_numbers_table_limma_down_wide[['direction']] <- 'down'

# merge
de_numbers_table_limma_wide <- rbind(de_numbers_table_limma_down_wide, de_numbers_table_limma_up_wide)
# make the one against controls
de_numbers_table_limma_wide_controls <- data.frame(cell_type = de_numbers_table_limma_wide[de_numbers_table_limma_wide$combination == 'UTBaseline', 'cell_type'],
                                                   combination = rep('C', times = nrow(de_numbers_table_limma_wide[de_numbers_table_limma_wide$combination == 'UTBaseline', ])),
                                                   number = de_numbers_table_limma_wide[de_numbers_table_limma_wide$combination == 'UTBaseline', 'number'],
                                                   direction = de_numbers_table_limma_wide[de_numbers_table_limma_wide$combination == 'UTBaseline', 'direction'])
plot_de_number_bars(de_numbers_table_limma_wide_controls, grid_single = F, use_label_dict = T, use_colour_coding_dict = T, angle_x_labels = T) + ylab('Number of DE genes') + xlab('Timepoint vs STEMI t0') + guides(fill = guide_legend(title = 'cell type direction'))
# and the one against the others
de_numbers_table_limma_wide_stemi <- de_numbers_table_limma_wide[de_numbers_table_limma_wide$combination %in% c('t24hBaseline', 't8wBaseline'), ]
de_numbers_table_limma_wide_stemi[['against']] <- NA
de_numbers_table_limma_wide_stemi[de_numbers_table_limma_wide_stemi$combination == 't24hBaseline', 'against'] <- 't24h'
de_numbers_table_limma_wide_stemi[de_numbers_table_limma_wide_stemi$combination == 't8wBaseline', 'against'] <- 't8w'
plot_de_number_bars(de_numbers_table_limma_wide_stemi, combination_column = 'against', grid_single = F, use_label_dict = T, use_colour_coding_dict = T, angle_x_labels = T) + ylab('Number of DE genes') + xlab('Timepoint vs STEMI t0') + guides(fill = guide_legend(title = 'cell type direction'))
