...
get_output_per_comparison <- function(output_loc, cell_types=c('B', 'CD4T', 'CD8T', 'DC', 'monocyte', 'NK'), comparisons=c('UTBaseline', 'Baselinet24h', 't24ht8w')) {
...
write_output_to_excel <- function(de_output_loc, excel_output_loc, cell_types=c('B', 'CD4T', 'CD8T', 'DC', 'monocyte', 'NK'), comparisons=c('UTBaseline', 'Baselinet24h', 't24ht8w'), use_label_dict=T, remove_columns=c('gene_v2', 'gene_v3', 'idx')) {
  # get the output
  results_per_celltype <- get_output_per_comparison(de_output_loc, cell_types, comparisons)
  ...
  for (celltype in names(results_per_celltype)) {
    ...
    if (use_label_dict) {
      ...
      celltype_result[['comparison']] <- as.vector(unlist(label_dict()[celltype_result[['comparison']]]))
    }
...
# location of the DE
de_output_loc <- '/groups/umcg-franke-scrna/tmp01/releases/blokland-2020/v1/differential_expression/MAST/stemi_meta_paired_lores_lfc01minpct01ncountrna_20210301/rna/'
# location of the Excel sheet
excel_loc <- '/groups/umcg-franke-scrna/tmp01/releases/blokland-2020/v1/differential_expression/MAST/stemi_meta_paired_lores_lfc01minpct01ncountrna_20210301_summary.xlsx'
# create the sheet
write_output_to_excel(de_output_loc, excel_loc, cell_types = c('bulk', 'B', 'CD4T', 'CD8T', 'DC', 'monocyte', 'NK'))
