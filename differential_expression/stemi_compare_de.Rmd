...
plot_concordance <- function(de_table, set_1_column, set_2_column, pointless=F, legendless=F, ylim=NULL, xlim=NULL, paper_style=T) {
  ...
  # get the location of where to plot the text
  x_pos_text <- max(abs(de_table[[set_1_column]])) * -.92
  y_pos_text <- max(abs(de_table[[set_2_column]])) * .97
  ...
  p <- ggplot(data = NULL, mapping = aes(x = de_table[[set_1_column]], y = de_table[[set_2_column]], color = de_table[['concordant']])) +
    ...
    annotate('text', x=x_pos_text, y=y_pos_text, color = 'black', label = round(concordance, digits = 2), hjust=1)
  ...
  if(!is.null(ylim)){
    ...
    p <- p + ylim(c(-1 * max(abs(de_table[[set_2_column]])), max(abs(de_table[[set_2_column]]))))
  }
  # and y limit
  if(!is.null(xlim)){
    p <- p + xlim(xlim)
    ...
    p <- p + xlim(c(-1 * max(abs(de_table[[set_1_column]])), max(abs(de_table[[set_1_column]]))))
  }
  if(pointless){
...
get_de_effect_sizes <- function(gene_table_1, gene_table_2, gene_column_1, gene_column_2, lfc_column_1, lfc_column_2, significance_column_1, significance_column_2, comparison_column_1='combination', comparison_column_2='combination', cell_type_column_1='cell_type', cell_type_column_2='cell_type', name_1='table_1', name_2='table_2', significance_cutoff_1=0.05, significance_cutoff_2=0.05, lfc_cutoff_1=NULL, lfc_cutoff_2=NULL, sign_flip_combinations_1=c(), sign_flip_combinations_2=c(), only_tested_both=F, na_to_zero=F) {
  # we will store the genes per for each combination, per comparison
  genes_per_combination_per_set <- list()
  # we will check each combination
  combinations_both <- intersect(unique(gene_table_1[[comparison_column_1]]), unique(gene_table_2[[comparison_column_2]]))
  for (combination in combinations_both) {
    ...
    for (cell_type in cell_types_both) {
      ...
      # check which were significant
      if (!is.null(lfc_cutoff_1) & !is.null(lfc_cutoff_2)) {
        genes_sig_1 <- genes_1[!is.na(genes_1[[significance_column_1]]) & 
                                 genes_1[[significance_column_1]] < significance_cutoff_1 &
                                 !is.na(genes_1[[lfc_column_1]]) &
                                 abs(genes_1[[lfc_column_1]]) >= lfc_cutoff_1, 
                               gene_column_1]
        genes_sig_2 <- genes_2[!is.na(genes_2[[significance_column_2]]) & 
                                 genes_2[[significance_column_2]] < significance_cutoff_2 &
                                 !is.na(genes_2[[lfc_column_2]]) &
                                 abs(genes_2[[lfc_column_2]]) >= lfc_cutoff_2, 
                               gene_column_2]
      }
      else{
...
plot_effect_sizes <- function(de_table, set_1_column, set_2_column, category_column=NULL, pointless=F, legendless=F, ylim=NULL, xlim=NULL, paper_style=T, category_order=NULL, calculate_concordance=F, inclusion_values_per_column=NULL) {
  # create the plot
  p <- NULL
  # with a catagory or not
  if (!is.null(category_column)) {
    # order the category if requested
    if (!is.null(category_order)) {
      de_table_list <- list()
      # by checking each category
      for (category in category_order) {
        de_table_category <- de_table[de_table[[category_column]] == category, ]
        de_table_list[[category]] <- de_table_category
      }
      # add the ones we didn't check last
      de_table_list[['leftover']] <- de_table[!(de_table[[category_column]] %in% category_order), ]
  ...
  if(!is.null(ylim)){
    ...
    ylim_use <- (c(-1 * max(abs(de_table[[set_2_column]])), max(abs(de_table[[set_2_column]]))))
  }
  p <- p + ylim(ylim_use)
  # and y limit
  xlim_use <- NULL
  if(!is.null(xlim)){
   xlim_use <- xlim
    ...
    xlim_use <- (c(-1 * max(abs(de_table[[set_1_column]])), max(abs(de_table[[set_1_column]]))))
  }
  p <- p + xlim(xlim_use)
  if(pointless){
    p <- p + theme(axis.text.x=element_blank(), 
  ...
  # add concordance if we asked for it
  if (calculate_concordance) {
    # we'll take the data we need to calculate concordance on
    data_for_concordance <- de_table
    # subset data if requested
    if (!is.null(inclusion_values_per_column)) {
      # check each column
      for (column in names(inclusion_values_per_column)) {
        # get the associated values
        data_for_concordance <- data_for_concordance[data_for_concordance[[column]] %in% inclusion_values_per_column[[column]], ]
      }
    }
    # get the signs
    data_for_concordance[['sign_1']] <- sign(data_for_concordance[[set_1_column]])
    data_for_concordance[['sign_2']] <- sign(data_for_concordance[[set_2_column]])
    # get the fraction of ones with the same sign
    same_sign_prop <- sum(data_for_concordance[['sign_1']] == data_for_concordance[['sign_2']]) / nrow(data_for_concordance)
    # add to plot
    x_pos_text <- .875 * xlim_use[1]
    y_pos_text <- .925 * ylim_use[2]
    p <- p + annotate('text', x=x_pos_text, y=y_pos_text, color = 'black', label = paste('concordance', round(same_sign_prop, digits = 2), sep = '\n'), hjust=0.5)
  }
  return(p)
}
...
get_de_significance_overlaps <- function(de_table_both, lfc_column_1, lfc_column_2, tested_column='tested', significant_column='significant', combination_column='combination', cell_type_column='cell_type') {
  ...
  for (combination in combinations) {
    ...
    for (cell_type in cell_types) {
      ...
      # counting
      sig_same <- nrow(de_table_condition_celltype[de_table_condition_celltype[[significant_column]] == 'both' & de_table_condition_celltype[[tested_column]] == 'both' & de_table_condition_celltype[['sign1']] == de_table_condition_celltype[['sign2']], ])
      sig_dif <- nrow(de_table_condition_celltype[de_table_condition_celltype[[significant_column]] == 'both' & de_table_condition_celltype[[tested_column]] == 'both' & de_table_condition_celltype[['sign1']] != de_table_condition_celltype[['sign2']], ])
...
plot_grid(
  # CD4T Baseline vs UT
  plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD4T C vs t0 effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD4T C vs t0 tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('CD4T C vs t0 significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
  
  # CD4T t24h Baseline
  plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD4T t0 vs t24h effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD4T t0 vs t24h tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('CD4T t0 vs t24h significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
  
  # CD4T t8w Baseline
   plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD4T t0 vs t8w effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD4T t0 vs t8w tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('CD4T t0 vs t8w significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
...
plot_grid(
  ...
  plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('monocyte C vs t0 effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('monocyte C vs t0 tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('monocyte C vs t0 significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
  
  # monocyte t24h Baseline
  plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('monocyte t0 vs t24h effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('monocyte t0 vs t24h tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('monocyte t0 vs t24h significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
  
  # monocyte t8w Baseline
   plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('monocyte t0 vs t8w effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('monocyte t0 vs t8w tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('monocyte t0 vs t8w significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
...
plot_grid(
  # B Baseline vs UT
  plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('B C vs t0 effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('B C vs t0 tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('B C vs t0 significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
  
  # B t24h Baseline
  plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('B t0 vs t24h effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('B t0 vs t24h tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('B t0 vs t24h significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
  
  # B t8w Baseline
   plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('B t0 vs t8w effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('B t0 vs t8w tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('B t0 vs t8w significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
...
plot_grid(
  # CD8T Baseline vs UT
  plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD8T C vs t0 effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD8T C vs t0 tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('CD8T C vs t0 significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
  
  # CD8T t24h Baseline
  plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD8T t0 vs t24h effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD8T t0 vs t24h tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither')) + ggtitle('CD8T t0 vs t24h significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
  
  # CD8T t8w Baseline
   plot_concordance(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T' & unsigned_mast_vs_limma_both_effect_sizes$tested == 'both', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD8T t0 vs t8w effect concordance') + scale_color_manual(values = list('concordant' = 'darkgreen', 'disconcordant' = 'darkred')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'tested', legendless = F, xlim = NULL, ylim=NULL) + ggtitle('CD8T t0 vs t8w tested in') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2')),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=F, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('CD8T t0 vs t8w significance') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray')),
...
```{r concordance_limma_both_mast_unsigned_last, include=TRUE, fig.width=20, fig.height=10}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

plot_grid(
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('B C vs t0') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('B t0 vs t24h') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'B', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('B t0 vs t8w') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('CD4+T C vs t0') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('CD4+T t0 vs t24h') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD4T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('CD4+T t0 vs t8w') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('CD8+T C vs t0') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('CD8+T t0 vs t24h') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'CD8T', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('CD8+T t0 vs t8w') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'DC', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('DC C vs t0') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'DC', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('DC t0 vs t24h') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'DC', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('DC t0 vs t8w') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('monocyte C vs t0') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('monocyte t0 vs t24h') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'monocyte', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('monocyte t0 vs t8w') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'NK', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('NK C vs t0') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'NK', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('NK t0 vs t24h') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 't8w-Baseline' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'NK', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = T, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + ggtitle('NK t0 vs t8w') + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither')) + xlab('LFC MAST') + ylab('LFC limma'),
  
  get_legend(plot_effect_sizes(unsigned_mast_vs_limma_both_effect_sizes[unsigned_mast_vs_limma_both_effect_sizes$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_sizes$cell_type == 'NK', ], set_1_column = 'mast_meta_unsigned', set_2_column = 'limma_both', category_column = 'significant', legendless = F, xlim = NULL, ylim=NULL, category_order = c('neither'), calculate_concordance=T, inclusion_values_per_column=list('tested' = c('both'))) + scale_color_manual(values = list('both' = 'darkgreen', 'limma_both' = 'gold2', 'mast_meta_unsigned' = 'blue', 'neither' = 'gray'), labels = c('both', 'limma', 'MAST', 'neither'))), 
...
# plot them
plot_grid(
  plot_de_significance_overlaps(unsigned_mast_vs_limma_both_effect_overlaps[unsigned_mast_vs_limma_both_effect_overlaps$combination == 'Baseline-UT' & unsigned_mast_vs_limma_both_effect_overlaps$classification != 'tested only limma_both and not significant', ], number_column = 'proportion') + scale_fill_manual(values = classification_color_list) + ggtitle('C-t0 DE gene overlap'),
  plot_de_significance_overlaps(unsigned_mast_vs_limma_both_effect_overlaps[unsigned_mast_vs_limma_both_effect_overlaps$combination == 't24h-Baseline' & unsigned_mast_vs_limma_both_effect_overlaps$classification != 'tested only limma_both and not significant', ], number_column = 'proportion') + scale_fill_manual(values = classification_color_list) + ggtitle('t0-t24h DE gene overlap'),
  plot_de_significance_overlaps(unsigned_mast_vs_limma_both_effect_overlaps[unsigned_mast_vs_limma_both_effect_overlaps$combination == 't8w-t24h' & unsigned_mast_vs_limma_both_effect_overlaps$classification != 'tested only limma_both and not significant', ], number_column = 'proportion') + scale_fill_manual(values = classification_color_list) + ggtitle('t24h-t8w DE gene overlap'),
  plot_de_significance_overlaps(unsigned_mast_vs_limma_both_effect_overlaps[unsigned_mast_vs_limma_both_effect_overlaps$combination == 't8w-Baseline' & unsigned_mast_vs_limma_both_effect_overlaps$classification != 'tested only limma_both and not significant', ], number_column = 'proportion') + scale_fill_manual(values = classification_color_list) + ggtitle('t0-t8w DE gene overlap')
)
