...
pathway_names_to_top_level <- function(pathway_mappings, pathway_names) {
  ...
  for (top_level in top_level_pathways) {
    ...
    # make dataframe
    pathway_mapping_category <- data.frame(pathway = filtered_names, id = filtered_ids, top_parent = rep(parent_name, times = length(parent_name)))
    # add to list
...
# load the pathways
pathways <- read.table('/groups/umcg-franke-scrna/tmp01/releases/blokland-2020/v1/pathway_enrichment//ReactomePathways.tsv', sep='\t')
# subset to just human to speed up the search
...
filtered_names <- get_filtered_pathway_names(pathways, pathway_mappings, 'R-HSA-168256', add_id = T)
# get the top level human pathways
...
# get all output
limma_both_enrichr_baseline_vs_t24ht8w <- get_plotframe_per_celltype(limma_both_enrichr, reference = 'Baseline', compare_tos = c('t24h', 't8w'), term_column = 'Term', significance_column = 'Old.Adjusted.P.value', filter_vector = filtered_names)
# make plots
...
# get all output
limma_both_enrichr_negative <- get_pathways_per_celltype_condition(pathway_output_loc, pval_cutoff = 0.05, pval_column = 'Old.Adjusted.P.value', append = '_negative.tsv', cell_types = c('monocyte'))

# get all output
limma_both_enrichr_ut_vs_baseline_down <- get_plotframe_per_celltype(limma_both_enrichr_negative, reference = 'Baseline', compare_tos = c('UT'), term_column = 'Term', significance_column = 'Old.Adjusted.P.value', filter_vector = filtered_names)
limma_both_enrichr_ut_vs_baseline_up <- get_plotframe_per_celltype(limma_both_enrichr, reference = 'Baseline', compare_tos = c('UT'), term_column = 'Term', significance_column = 'Old.Adjusted.P.value', filter_vector = filtered_names)
...
category_colors <- as.list(sample_many_colours(n = length(unique(pathways_to_top_level[['top_parent_readable']])), color_indices = c(64:65, 8, 66:72, 9, 12, 46:48, 2, 13, 49:54, 5)))

names(category_colors) <- unique(pathways_to_top_level[['top_parent_readable']])

plot_grid(
  plot_pathway_categories_celltype(limma_both_enrichr[limma_both_enrichr$cell_type == 'monocyte' & limma_both_enrichr$condition2=='Baseline' & limma_both_enrichr$condition1 == 'UT', ], pathways_to_top_level, use_distinct_colours = T, legendless = T, drop_categories = c('unannotated'), parent_column_mapping_table = 'top_parent_readable') + ggtitle('pathway categories monocytes C vs t0') + scale_fill_manual(values = category_colors),
  plot_pathway_categories_celltype(limma_both_enrichr[limma_both_enrichr$cell_type == 'monocyte' & limma_both_enrichr$condition2=='Baseline' & limma_both_enrichr$condition1 == 't24h', ], pathways_to_top_level, use_distinct_colours = T, legendless = T, drop_categories = c('unannotated'), parent_column_mapping_table = 'top_parent_readable') + ggtitle('pathway categories monocytes t0 vs t24h') + scale_fill_manual(values = category_colors),
#  plot_pathway_categories_celltype(limma_both_enrichr[limma_both_enrichr$cell_type == 'monocyte' & limma_both_enrichr$condition2=='t24h' & limma_both_enrichr$condition1 == 't8w', ], pathways_to_top_level, use_distinct_colours = T, legendless = T, drop_categories = c('unannotated'), parent_column_mapping_table = 'top_parent_readable') + ggtitle('pathway categories monocytes t24h vs t8w') + scale_fill_manual(values = category_colors),
  plot_pathway_categories_celltype(limma_both_enrichr[limma_both_enrichr$cell_type == 'monocyte' & limma_both_enrichr$condition2=='Baseline' & limma_both_enrichr$condition1 == 't8w', ], pathways_to_top_level, use_distinct_colours = T, legendless = T, drop_categories = c('unannotated'), parent_column_mapping_table = 'top_parent_readable') + ggtitle('pathway categories monocytes t0 vs t8w') + scale_fill_manual(values = category_colors),
ncol = 3
)

```

```{r limma_kegg, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

# get all output
limma_both_enrichr_kegg_positive <- get_pathways_per_celltype_condition(enrichr_kegg_output_loc, pval_cutoff = 1.1, append = '_positive.tsv')
...
# get all output
limma_both_enrichr_kegg_baseline_vs_t24ht8w_positive <- get_plotframe_per_celltype(limma_both_enrichr_kegg_positive, reference = 'Baseline', compare_tos = c('t24h', 't8w'), top_so_many = 10)
# make plots
plot_pathways_celltype(limma_both_enrichr_kegg_baseline_vs_t24ht8w_positive[['monocyte']], legendless = T, set_size_outside = T)

```

```{r limma_clusterprofiler, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

# get all output
limma_both_clusterprofiler_kegg_positive <- get_pathways_per_celltype_condition(clusterprofiler_kegg_output_loc, pval_cutoff = 1.1, pval_column = 'qvalue', direction_column='enrichmentScore', only_positive=T, only_negative=F)

```

```{r limma_clusterprofiler_plots, include=TRUE, fig.width=10, fig.height=10}
knitr::opts_chunk$set(echo = FALSE)

# get all output
limma_both_clusterprofiler_kegg_baseline_vs_t24ht8w_positive <- get_plotframe_per_celltype(limma_both_clusterprofiler_kegg_positive[limma_both_clusterprofiler_kegg_positive$cell_type %in% c('monocyte'), ], reference = 'Baseline', compare_tos = c('t24h'), significance_column = 'qvalue', reference_column = 'condition2', compare_column = 'condition1', top_so_many = 10)
# make plots
plot_pathways_celltype(limma_both_clusterprofiler_kegg_baseline_vs_t24ht8w_positive[['monocyte']], legendless = T, set_size_outside = T, term_column = 'Description', set_size_column = 'enrichmentScore')
