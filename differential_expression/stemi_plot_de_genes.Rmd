...
lfc_table_to_tile <- function(lfc_table, gene_column='gene', comparison_column='vs_timepoint', value_column='avg_lfc', use_label_mapping=T, paper_style=T, print_lfc_value=T, value_print_column=NULL, round_digits=2, align_right=T, legendless=F, colour_ticks=F, x_order=NULL, add_non_combinations=F) {
  ...
  p <- p + scale_fill_gradientn(colours = c('darkblue', 'white', 'darkred'),  limits = (c(-1 * max_abs_lfc, max_abs_lfc)), name = 'average\nLFC')
...
```{r limma_dendro, include=TRUE, fig.width=7, fig.height=7}
knitr::opts_chunk$set(echo = FALSE)

# take the genes in the data
genes_mono <- unique(mono_table[['gene']])
# create table
mono_wide <- data.frame(UT_Baseline = mono_table[mono_table[['comparison']] == 'UT_Baseline', ][match(genes_mono, mono_table[mono_table[['comparison']] == 'UT_Baseline', 'gene']), 'logFC'],
                        t24h_Baseline = mono_table[mono_table[['comparison']] == 't24h_Baseline', ][match(genes_mono, mono_table[mono_table[['comparison']] == 't24h_Baseline', 'gene']), 'logFC'],
                        t8w_t24h = mono_table[mono_table[['comparison']] == 't8w_t24h', ][match(genes_mono, mono_table[mono_table[['comparison']] == 't8w_t24h', 'gene']), 'logFC'])
rownames(mono_wide) <- genes_mono
# do ggdendro hierarchial clustering and create the dendogram
mono_model <- hclust(dist(mono_wide), "ave")
mono_dhc <- as.dendrogram(mono_model)
# get the data from the dendogram
mono_ddata <- dendro_data(mono_dhc, type = "rectangle")
# plot the dendogram
mono_dendro_p <- ggplot(segment(mono_ddata)) + 
...
mono_dendro_gene_order <- mono_ddata$labels$label
# set ordered genes
mono_table[['dendro_ordered_genes']] <- factor(mono_table[['gene']], levels = rev(mono_dendro_gene_order))

# create a picture
plot_grid(
  mono_dendro_p + ggtitle('top DE genes versus t0 in monocytes'),
  lfc_table_to_tile(mono_table, gene_column = 'dendro_ordered_genes', print_lfc_value = T, colour_ticks = T, comparison_column = 'comparison', value_column = 'logFC', value_print_column = 'plot_number_lfc', x_order = rev(c('C-t0', 't0-t24h', 't24h-t8w'))) + ylab('gene symbol') + xlab('timepoint compared') + coord_flip(),
  nrow = 2
)
```
